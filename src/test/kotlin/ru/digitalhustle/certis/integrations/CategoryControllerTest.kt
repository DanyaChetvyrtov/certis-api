...
import ru.digitalhustle.certis.features.category.constants.CategoryErrorMessages
...
class CategoryControllerTest : AbstractIntegrationTest() {
    ...
    fun `should reject update for archived category`() {
        ...
        mvc.perform(
            ...
            // then
            .andExpect(status().isConflict)
            .andExpect(jsonPath("$.message").value(CategoryErrorMessages.CATEGORY_ARCHIVED))
    }
    ...
    fun `should reject archive for category used by active recurring template`() {
        ...
        mvc.perform(
            ...
            // then
            .andExpect(status().isConflict)
            .andExpect(jsonPath("$.message").value(CategoryErrorMessages.CATEGORY_IN_USE))
