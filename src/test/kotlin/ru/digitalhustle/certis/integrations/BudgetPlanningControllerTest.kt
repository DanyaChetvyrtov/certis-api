...
class BudgetPlanningControllerTest : AbstractIntegrationTest() {
    ...
    @Test
    fun `should cancel only a current draft plan`() {
        val user = userFixture.createInDb()
        val plan = getBody(
            createPlan(user, IDEMPOTENCY_KEY).andExpect(status().isCreated),
            BudgetPlanRs::class.java,
        ...
        mvc.perform(
            put(cancellationPath(plan.id))
                .cookie(accessTokenCookie(user))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsBytes(CancelBudgetPlanRq(expectedVersion = 0))),
        )
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.status").value("CANCELLED"))
            .andExpect(jsonPath("$.version").value(1))
            .andExpect(jsonPath("$.capabilities.canEditForecast").value(false))
            .andExpect(jsonPath("$.capabilities.canCancel").value(false))

        mvc.perform(
            put(cancellationPath(plan.id))
                .cookie(accessTokenCookie(user))
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsBytes(CancelBudgetPlanRq(expectedVersion = 1))),
        )
            .andExpect(status().isConflict)
            .andExpect(jsonPath("$.code").value(BudgetPlanningErrorCode.INVALID_BUDGET_PLAN_STATE.name))
    }
    ...
    private fun cancellationPath(planId: UUID): String =
        PathConstants.BUDGET_PLANS +
            PathConstants.BUDGET_PLAN_CANCELLATION.replace("{planId}", planId.toString())
