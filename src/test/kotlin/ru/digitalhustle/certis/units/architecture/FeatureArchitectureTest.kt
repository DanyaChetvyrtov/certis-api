...
class FeatureArchitectureTest {
    ...
    @Test
    fun `features must be free of cyclic dependencies`() {
    ...
    @Test
    fun `shared code must not depend on features or HTTP API`() {
    ...
    private companion object {
        private const val ROOT = "ru.digitalhustle.certis"
        private const val FEATURES = "$ROOT.features"
        private const val API = "$ROOT.api"
        private const val SHARED = "$ROOT.shared"
        private val classes = ClassFileImporter()
            .withImportOption(ImportOption.DoNotIncludeTests())
            .importPackages("ru.digitalhustle.certis")
        private val PUBLIC_PACKAGES = setOf("api", "spi", "enums", "exceptions")
        ...
        private val OWNED_TABLES = mapOf(
            ...
            "BudgetForecastRevisionRepository" to "BUDGET_FORECAST_REVISIONS",
            "BudgetForecastItemRepository" to "BUDGET_FORECAST_ITEMS",
            "BudgetConstraintRevisionRepository" to "BUDGET_CONSTRAINT_REVISIONS",
            "BudgetCategoryConstraintRepository" to "BUDGET_CATEGORY_CONSTRAINTS",
            "BudgetConstraintFundingLevelRepository" to "BUDGET_CONSTRAINT_FUNDING_LEVELS",
            "BudgetOptimizationRunRepository" to "BUDGET_OPTIMIZATION_RUNS",
            "BudgetOptimizationDecisionRepository" to "BUDGET_OPTIMIZATION_DECISIONS",
