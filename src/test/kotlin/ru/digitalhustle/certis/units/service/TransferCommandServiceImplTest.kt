...
import ru.digitalhustle.certis.features.transaction.command.service.impl.TransferCommandServiceImpl
...
class TransferCommandServiceImplTest {
    ...
    private val transferCommandService = TransferCommandServiceImpl(
        transferService,
        accountService,
        transactionService,
        TransferValidator(),
    ...
    fun `should save transfer and linked debit and credit postings`() {
        ...
        // when
        val result = transferCommandService.save(createData)
    ...
    fun `should reject transfer to the same account`() {
        ...
        // when, then
        assertThatThrownBy {
            transferCommandService.save(createData)
        }
            .isInstanceOf(InvalidTransferException::class.java)
            .hasMessage(TransactionErrorMessages.TRANSFER_SAME_ACCOUNT)
    ...
    fun `should reject transfer involving closed account`() {
        ...
        assertThatThrownBy {
            ...
            .hasMessage(TransactionErrorMessages.TRANSFER_ACCOUNT_CLOSED)
    ...
    fun `should reject transfer between different currencies`() {
        ...
        val destinationAccount = createAccount(currency = Currency.USD)
        val createData = createTransferData(sourceAccount, destinationAccount)
        `when`(
            accountService.getAllByIdsForShare(
                listOf(sourceAccount.id, destinationAccount.id),
        ...
        assertThatThrownBy {
            ...
            .isInstanceOf(InvalidTransferException::class.java)
            .hasMessage(TransactionErrorMessages.TRANSFER_CURRENCY_MISMATCH)
    ...
    fun `should reverse transfer with linked debit and credit postings`() {
        ...
        val originalDestination = createAccount()
        ...
        // when
        val result = transferCommandService.reverse(reverseData)
    ...
    fun `should reject reversal of a reversal`() {
        ...
        // when, then
        assertThatThrownBy {
            transferCommandService.reverse(reverseData)
        }
            .isInstanceOf(InvalidTransferException::class.java)
            .hasMessage(TransactionErrorMessages.TRANSFER_REVERSAL_OF_REVERSAL)
