...
class BudgetPlanRepository(
    ...
    fun apply(plan: BudgetPlan, budgetId: UUID, appliedAt: OffsetDateTime): BudgetPlan? =
        dsl.update(Tables.BUDGET_PLANS)
            .set(Tables.BUDGET_PLANS.APPLIED_BUDGET_ID, budgetId)
            .set(Tables.BUDGET_PLANS.STATUS, BudgetPlanStatus.APPLIED.name)
            .set(Tables.BUDGET_PLANS.VERSION, plan.version + 1)
            .set(Tables.BUDGET_PLANS.UPDATED_AT, appliedAt)
            .set(Tables.BUDGET_PLANS.APPLIED_AT, appliedAt)
            .where(
                Tables.BUDGET_PLANS.ID.eq(plan.id)
                    .and(Tables.BUDGET_PLANS.USER_ID.eq(plan.userId))
                    .and(Tables.BUDGET_PLANS.VERSION.eq(plan.version))
    ...
    fun cancel(plan: BudgetPlan, cancelledAt: OffsetDateTime): BudgetPlan? =
        dsl.update(Tables.BUDGET_PLANS)
            .set(Tables.BUDGET_PLANS.STATUS, BudgetPlanStatus.CANCELLED.name)
            .set(Tables.BUDGET_PLANS.VERSION, plan.version + 1)
            .set(Tables.BUDGET_PLANS.UPDATED_AT, cancelledAt)
            .set(Tables.BUDGET_PLANS.CANCELLED_AT, cancelledAt)
            .where(
                Tables.BUDGET_PLANS.ID.eq(plan.id)
                    .and(Tables.BUDGET_PLANS.USER_ID.eq(plan.userId))
                    .and(Tables.BUDGET_PLANS.VERSION.eq(plan.version))
    ...
    fun supersedeCurrentApplied(plan: BudgetPlan, supersededAt: OffsetDateTime) {
        dsl.update(Tables.BUDGET_PLANS)
            .set(Tables.BUDGET_PLANS.STATUS, BudgetPlanStatus.SUPERSEDED.name)
            .set(Tables.BUDGET_PLANS.VERSION, Tables.BUDGET_PLANS.VERSION.plus(1))
            .set(Tables.BUDGET_PLANS.UPDATED_AT, supersededAt)
            .set(Tables.BUDGET_PLANS.SUPERSEDED_AT, supersededAt)
            .where(
                scopeCondition(plan.userId, plan.budgetMonth, plan.currency)
                    .and(Tables.BUDGET_PLANS.STATUS.eq(BudgetPlanStatus.APPLIED.name)),
            )
            .execute()
    }
