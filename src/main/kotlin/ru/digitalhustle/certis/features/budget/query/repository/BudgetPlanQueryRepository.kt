...
class BudgetPlanQueryRepository(
    ...
    private fun findLatestConstraints(planIds: Collection<UUID>): Map<UUID, BudgetPlanConstraintSnapshot> {
        ...
        return dsl.select(*constraints.fields(), Tables.BUDGET_FORECAST_REVISIONS.REVISION)
            .from(constraints)
            .join(Tables.BUDGET_FORECAST_REVISIONS)
            .on(Tables.BUDGET_FORECAST_REVISIONS.ID.eq(constraints.FORECAST_REVISION_ID))
            .where(
            ...
            .fetch { record ->
                requireNotNull(record[constraints.PLAN_ID]) to BudgetPlanConstraintSnapshot(
                    ...
                    forecastRevision = requireNotNull(record[Tables.BUDGET_FORECAST_REVISIONS.REVISION]),
