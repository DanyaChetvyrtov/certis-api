...
import ru.digitalhustle.certis.features.goal.enums.GoalSort
...
import ru.digitalhustle.certis.features.goal.model.GoalStatusCounts
...
class GoalQueryRepository(
    ...
    private fun mapGoal(record: Record): Goal =
        Goal(
            id = checkNotNull(record[Tables.GOALS.ID]),
            userId = checkNotNull(record[Tables.GOALS.USER_ID]),
            name = checkNotNull(record[Tables.GOALS.NAME]),
            targetAmount = checkNotNull(record[Tables.GOALS.TARGET_AMOUNT]),
            currency = Currency.valueOf(checkNotNull(record[Tables.GOALS.CURRENCY])),
            deadline = record[Tables.GOALS.DEADLINE],
            contributionPlanType = GoalContributionPlanType.valueOf(
                checkNotNull(record[Tables.GOALS.CONTRIBUTION_PLAN_TYPE]),
            ),
            monthlyContributionAmount = checkNotNull(record[Tables.GOALS.MONTHLY_CONTRIBUTION_AMOUNT]),
            icon = checkNotNull(record[Tables.GOALS.ICON]),
            color = checkNotNull(record[Tables.GOALS.COLOR]),
            status = GoalStatus.valueOf(checkNotNull(record[Tables.GOALS.STATUS])),
            createdAt = checkNotNull(record[Tables.GOALS.CREATED_AT]),
            updatedAt = checkNotNull(record[Tables.GOALS.UPDATED_AT]),
    ...
    fun existsByIdAndUserId(id: UUID, userId: UUID): Boolean =
        dsl.fetchExists(
            dsl.selectOne()
                .from(Tables.GOALS)
                .where(Tables.GOALS.ID.eq(id).and(Tables.GOALS.USER_ID.eq(userId))),
        )
}
