...
| `spi` | Consumer-owned ports implemented by another feature to prevent dependency cycles |
| `constants` | Constants owned by one feature and not exposed as cross-feature contracts |
...
- Accounts expose `AccountCommandAccess` and a minimal immutable `AccountSnapshot`
  ...
  aggregate transactions and goal postings. Account owns
  `RecurringTransactionAccountUsage` as an SPI needed by its closure workflow;
  transaction implements it, so account does not depend on transaction code.
...
- Profiles split profile management from photo write workflows. The internal
  `ProfilePhotoLifecycleManager` owns photo processing, metadata/storage
  coordination and cleanup after commit/rollback; profile management depends on
  its narrower `ProfilePhotoSupport` contract. Profile and photo GETs use query
  services.
...
Provider-owned capabilities belong to `api`; consumer-owned ports used to invert
a dependency belong to `spi`. Query projections describe their own data; a join
grants no access to foreign code.

HTTP route fragments and other boundary-owned constants belong to
`api.constants`. Feature-specific messages and constraints belong to the owning
feature's `constants` package. Stable value types and policies reused by several
features belong to `shared`; shared code must not depend on features or the HTTP
API.
...
`FeatureArchitectureTest` verifies dependency direction, absence of feature
cycles, public boundaries, read-only services, query writes/locks, repository
access and table ownership.
