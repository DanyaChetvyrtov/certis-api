...
| `account.spi.RecurringTransactionAccountUsage` | Consumer-owned account port implemented by transaction for account closure checks |
...
- Command workflows call account/category public APIs for validation and shared
  locks. Transaction implements account's recurring-usage SPI, keeping the
  feature dependency direction `transaction -> account`. The extension to all
  features is described in `feature-command-query.md`.
