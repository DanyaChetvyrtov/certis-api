...
structure under `ru.digitalhustle.certis.features`, the shared HTTP boundary
under `ru.digitalhustle.certis.api`, and deliberately small cross-feature
building blocks under `ru.digitalhustle.certis.shared`.
...
- `api.constants`: route fragments and constants owned by the HTTP boundary;
...
- `features.<feature>.spi`: consumer-owned cross-feature ports used to invert a
  dependency that would otherwise create a feature cycle;
- `features.<feature>.model`: models owned by the feature and shared between its
  internal command, query, and application layers;
- `features.<feature>.constants`: constants owned by one feature and not exposed
  as cross-feature contracts;
- `features.<feature>.gateway`: adapters for external systems owned by the
  feature;
- `shared`: stable value types, policies, and constants deliberately reused by
  several features; shared code must not depend on a feature or the HTTP API;
...
shared package merely because another feature needs similar behavior. Keep the
shared namespace small and cohesive, and introduce purpose-revealing
subpackages when it grows instead of turning it into another dumping ground.
Keep ownership explicit and expose the minimum required capability through the
...
- Name cross-feature APIs after the capability they expose, such as
  `AccountCommandAccess` or `UserPreferencesQuery`; do not publish a feature's
  broad internal service interface.
- If depending on a provider-owned `api` would create a cycle, define a focused
  consumer-owned port in the consumer feature's `spi` package and implement it
...
- Keep cross-feature return models minimal. Do not expose an entire entity when
  ...
  snapshot.
- Define provider-owned API interfaces in the provider feature and
  consumer-owned SPI interfaces in the consumer feature. Implement each
  interface in the feature supplying the capability. Consumers must depend
  only on these interfaces and their public contract models.
...
- Dependencies point from feature and API code toward shared building blocks,
  never back from shared code toward their consumers.
...
- API constants, feature constants, and shared constants remain with their
  architectural owner; shared code has no feature or HTTP API dependencies;
- cross-feature calls use narrow interfaces and minimal projections from the
  provider feature's `api` package or consumer-owned ports from `spi`;
- feature dependencies are free of cycles;
